...
namespace WarcraftPlugin.Core
{
    internal class CooldownManager
    {
        ...
        private static void PlayEffects(WarcraftPlayer wcplayer, int abilityIndex)
        {
            ...
            var ability = warcraftClass?.GetAbility(abilityIndex);

            if (player == null || !player.IsValid || ability == null)
            {
                WarcraftPlugin.Instance?.CooldownManager?.UnregisterPlayer(wcplayer);
                return;
            }
