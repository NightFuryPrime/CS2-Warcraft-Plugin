    ...
    internal static class ClassMenu
    {
        internal static void Show(CCSPlayerController player, List<ClassInformation> classInformations)
        {
            ...
            var classInfoDict = classInformations?.ToDictionary(
                x => x.RaceName,
                x => x,
                StringComparer.OrdinalIgnoreCase) ?? new Dictionary<string, ClassInformation>(StringComparer.OrdinalIgnoreCase);
                ...
                .ThenBy(x => x.DisplayName))
            {
                ...
                var levelColor = TransitionToGold(warClassInformation.CurrentLevel / (float)WarcraftPlugin.MaxLevel);
