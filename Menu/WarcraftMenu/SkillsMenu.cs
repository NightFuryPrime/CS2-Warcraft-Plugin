...
namespace WarcraftPlugin.Menu.WarcraftMenu
{
    internal static class SkillsMenu
    {
        internal static void Show(WarcraftPlayer wcPlayer, int selectedOptionIndex = 0)
        {
            ...
            var builder = MenuBuilder.Create(@$"<font color='{warcraftClass.DefaultColor.AdjustBrightness(1.3f).ToHex()}' class='{FontSizes.FontSizeM}'>{warcraftClass.LocalizedDisplayName}</font><font color='gold' class='{FontSizes.FontSizeSm}'> - {plugin.Localizer["menu.skills.level"]} {wcPlayer.GetLevel()}</font><br>
                <font color='#90EE90' class='{FontSizes.FontSizeS}'>{plugin.Localizer["menu.skills.available", AbilityProgression.GetFreeSkillPoints(wcPlayer)]}</font>");
            ...
            for (int i = 0; i < warcraftClass.Abilities.Count; i++)
            {
                ...
                if (abilityLevel == maxAbilityLevel || AbilityProgression.GetFreeSkillPoints(wcPlayer) == 0)
                {
                    isDisabled = true;
                }
