    ...
    public abstract class WarcraftClass
    {
        ...
        public void SetDefaultAppearance()
        {
            RunWhenPawnReady(() =>
            {
                ...
                {
                    WarcraftPlugin.Instance?.DebugSetModel("SetDefaultAppearance", pawn, model);
                    pawn.SetModel(model);
                }
            });
        }

        protected void RunWhenPawnReady(Action action, int attempt = 0)
        {
            const int maxAttempts = 20;
            if (action == null || Player == null || !Player.IsValid)
                return;

            var pawn = Player.PlayerPawn?.Value;
            var ready = pawn != null && pawn.IsValid && pawn.CBodyComponent?.SceneNode?.GetSkeletonInstance() != null;
            if (!ready)
            {
                if (attempt >= maxAttempts) return;
                WarcraftPlugin.Instance?.DebugLog($"RunWhenPawnReady defer for {Player?.PlayerName} attempt {attempt + 1}");
                WarcraftPlugin.Instance?.AddTimer(0.05f, () => RunWhenPawnReady(action, attempt + 1));
                return;
            }
        ...
        protected HookBinder RegisterHooks() => new(this);
        protected AbilityBinder RegisterAbilities() => new(this);

        protected readonly struct HookBinder
        {
            private readonly WarcraftClass _class;
            internal HookBinder(WarcraftClass cls) => _class = cls;

            public HookBinder Hook<T>(Action<T> handler, HookMode mode = HookMode.Post) where T : GameEvent
            {
                _class.HookEvent(handler, mode);
                return this;
            }
        }

        protected readonly struct AbilityBinder
        {
            private readonly WarcraftClass _class;
            internal AbilityBinder(WarcraftClass cls) => _class = cls;

            public AbilityBinder Bind(int abilityIndex, Action handler)
            {
                _class.HookAbility(abilityIndex, handler);
                return this;
            }
        }
    }
