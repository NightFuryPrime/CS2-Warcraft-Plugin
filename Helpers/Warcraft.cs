...
namespace WarcraftPlugin.Helpers
{
    ...
    {
        private static readonly Func<CCSPlayerPawn, QAngle> EyeAnglesResolver = ResolveEyeAnglesGetter();
        private static readonly Action<CBasePlayerPawn, CTakeDamageInfo> TakeDamageInvoker = ResolveTakeDamageInvoker();

        private static Action<CBasePlayerPawn, CTakeDamageInfo> ResolveTakeDamageInvoker()
        {
            var vfType = typeof(VirtualFunctions);
            string[] candidates =
            [
            ...
            {
                var directField = vfType.GetField(baseName, BindingFlags.Public | BindingFlags.Static);
                if (directField?.GetValue(null) is Delegate directDelegate)
                {
                    var method = directDelegate.Method;
                    var target = directDelegate.Target;
                    return BuildTakeDamageInvoker(target, method);
                }

                var funcField = vfType.GetField($"{baseName}Func", BindingFlags.Public | BindingFlags.Static);
                var funcValue = funcField?.GetValue(null);
                if (funcValue != null)
                {
                    var invoke = funcValue.GetType().GetMethod("Invoke", BindingFlags.Public | BindingFlags.Instance);
                    if (invoke != null)
                    {
                        return BuildTakeDamageInvoker(funcValue, invoke);
                    }
                }
            }
        ...
        private static Action<CBasePlayerPawn, CTakeDamageInfo> BuildTakeDamageInvoker(object target, MethodInfo method)
        {
            if (method == null)
                return (_, _) => { };
            ...
            return (pawn, info) =>
            {
                ...
                for (int i = 0; i < parameters.Length; i++)
                {
                    ...
                    if (paramType.IsInstanceOfType(pawn))
                    {
                        args[i] = pawn;
                    }
                    else if (typeof(CBaseEntity).IsAssignableFrom(paramType) || typeof(CEntityInstance).IsAssignableFrom(paramType))
                    {
                        args[i] = pawn;
                    }
                    else if (info != null && paramType.IsInstanceOfType(info))
                    {
                        args[i] = info;
                    }
                    else if (info != null && paramType.IsAssignableFrom(typeof(CTakeDamageInfo)))
                    {
                        args[i] = info;
                    }
                    else
        ...
        private static Func<CCSPlayerPawn, QAngle> ResolveEyeAnglesGetter()
        {
            QAngle DefaultAngle(CCSPlayerPawn pawn) => pawn?.AbsRotation ?? new QAngle();

            try
            {
                var pawnType = typeof(CCSPlayerPawnBase);

                var eyeAnglesProp = pawnType.GetProperty("EyeAngles", BindingFlags.Public | BindingFlags.Instance);
                if (eyeAnglesProp != null)
                {
                    return pawn =>
                    {
                        if (pawn == null) return new QAngle();
                        if (eyeAnglesProp.GetValue(pawn) is QAngle qAngle)
                            return qAngle;
                        return pawn.AbsRotation;
                    };
                }

                var eyeRotationProp = pawnType.GetProperty("EyeRotation", BindingFlags.Public | BindingFlags.Instance);
                if (eyeRotationProp != null)
                {
                    return pawn =>
                    {
                        if (pawn == null) return new QAngle();
                        if (eyeRotationProp.GetValue(pawn) is QAngle qAngle)
                            return qAngle;
                        return pawn.AbsRotation;
                    };
                }
        ...
        public static Vector EyePosition(this CCSPlayerController player, float offset = 0)
        {
            ...
            if (pawn == null) return new Vector();
            return pawn.AbsOrigin.Clone().Add(z: player.EyeHeight() + offset);
        }

        public static QAngle GetEyeAngles(this CCSPlayerPawn pawn)
        {
            if (pawn == null) return new QAngle();
            try
            {
                return EyeAnglesResolver(pawn) ?? pawn.AbsRotation;
            }
            catch
            {
                return pawn.AbsRotation;
        ...
        public static QAngle GetEyeAngles(this CCSPlayerController player)
        {
            return player?.PlayerPawn?.Value?.GetEyeAngles() ?? new QAngle();
        }

        /// <summary>
        /// Gets the world eye position of a pawn. Useful when only pawn handles are available.
        /// </summary>
        /// <param name="pawn">The pawn instance.</param>
        /// <param name="offset">Optional height offset.</param>
        public static Vector EyePosition(this CCSPlayerPawn pawn, float offset = 0)
        {
            if (pawn == null) return new Vector();
            var viewOffset = pawn.ViewOffset?.Z ?? 0f;
            return pawn.AbsOrigin.Clone().Add(z: viewOffset + offset);
        }
        ...
        public static void SpawnExplosion(Vector pos, float damage, float radius, CCSPlayerController attacker = null, KillFeedIcon? killFeedIcon = null)
        {
            ...
            {
                heProjectile.AcceptInput("InitializeSpawnFromWorld", attackerPawn, attackerPawn, "");
        ...
        public static CSmokeGrenadeProjectile SpawnSmoke(Vector pos, CCSPlayerPawn attacker, Color color)
        {
            ...
            if (attacker != null)
            {
                if (attacker.Controller?.Value is CCSPlayerController thrower && thrower.PlayerPawn != null)
                {
                    Schema.SetSchemaValue(smokeProjectile.Handle, "CBaseGrenade", "m_hThrower", thrower.PlayerPawn.Raw);
                }
            }
        ...
        /// <summary>
        /// Returns true if the random roll succeeds against the supplied percentage.
        /// </summary>
        public static bool RollChance(float percent)
        {
            ...
            return RandomProvider.NextDouble() * 100f < percent;
        }

        /// <summary>
        /// Returns true if the random roll succeeds given numerator/denominator (e.g., 1/3 chance).
        /// </summary>
        public static bool RollFraction(int numerator, int denominator)
        {
            if (denominator <= 0 || numerator <= 0) return false;
            return RandomProvider.Next(denominator) < numerator;
        }

        /// <summary>
        /// Gives the specified weapon to the player and wraps it in the desired type.
        /// </summary>
        /// <typeparam name="T">Weapon wrapper type (must inherit CBasePlayerWeapon).</typeparam>
        /// <param name="player">Target player.</param>
        /// <param name="designerName">Designer name (e.g., weapon_molotov).</param>
        /// <returns>The wrapped weapon instance or null if creation fails.</returns>
        public static CBasePlayerWeapon GiveWeapon(this CCSPlayerController player, string designerName)
        {
            if (player == null || !player.IsValid || string.IsNullOrWhiteSpace(designerName))
                return null;

            try
            {
                var handle = player.GiveNamedItem(designerName);
                if (handle == IntPtr.Zero)
                    return null;

                return new CBasePlayerWeapon(handle);
            }
            catch
            {
                return null;
        ...
        public static T GiveWeapon<T>(this CCSPlayerController player, string designerName) where T : CBasePlayerWeapon
        {
            var baseWeapon = GiveWeapon(player, designerName);
            if (baseWeapon == null)
                return null;

            try
            {
                return Activator.CreateInstance(typeof(T), baseWeapon.Handle) as T;
            }
            catch
            {
                return null;
        ...
        {
            if (player?.PlayerPawn?.Value == null || otherPlayer?.PlayerPawn?.Value == null) return false;
        ...
        /// <param name="player">The player controller.</param>
        /// <param name="soundPath">The path to the sound file.</param>
        /// <param name="volume">Optional volume scalar between 0-1. Defaults to 1 (full volume).</param>
        public static void PlayLocalSound(this CCSPlayerController player, string soundPath, float volume = 1f)
        {
            if (player == null || !player.IsValid) return;

            var clampedVolume = Math.Clamp(volume, 0f, 1f);
            var command = clampedVolume < 0.999f
                ? $"playvol {soundPath} {clampedVolume.ToString("0.00", CultureInfo.InvariantCulture)}"
                : $"play {soundPath}";
        ...
        public static Vector CalculateVelocityAwayFromPlayer(this CCSPlayerController player, int speed)
        {
            ...
            float pitchAngleRadians = (float)(eyeAngles.X * Math.PI / 180.0);
        ...
        public static Vector CalculatePositionInFront(this CCSPlayerController player, Vector offset)
        {
            ...
            float yawAngle = pawn.GetEyeAngles().Y;
            ...
            string abilityName = null, DamageTypes damageType = DamageTypes.Generic)
        {
            ...
            try
            {
                ...
                var effectiveAttacker = attacker != null && attacker.IsValid ? attacker : null;
                var attackerInfoSource = effectiveAttacker ?? victim;
                var attackerInfo = new Struct.CAttackerInfo(attackerInfoSource);

                Marshal.StructureToPtr(attackerInfo, new IntPtr(ptr.ToInt64() + 0x98), false);

                var attackerPawnRaw = effectiveAttacker?.Pawn?.Raw ?? victim.Pawn.Raw;
                var inflictorRaw = inflictor?.Pawn?.Raw ?? attackerPawnRaw;

                Schema.SetSchemaValue(damageInfo.Handle, "CTakeDamageInfo", "m_hInflictor", inflictorRaw);
                Schema.SetSchemaValue(damageInfo.Handle, "CTakeDamageInfo", "m_hAttacker", attackerPawnRaw);

                damageInfo.Damage = damage;
                damageInfo.NumObjectsPenetrated = penetrationKill ? 1 : 0;

                if (damageType != DamageTypes.Generic)
                {
                    Schema.SetSchemaValue(damageInfo.Handle, "CTakeDamageInfo", "m_bitsDamageType", (uint)damageType);
                }

                var attackerClass = effectiveAttacker?.GetWarcraftPlayer()?.GetClass();
                ...
                if (!string.IsNullOrEmpty(abilityName) && damage > 0)
                {
                    if (effectiveAttacker != null && effectiveAttacker.IsValid)
                        effectiveAttacker.PrintToChat($" {WarcraftPlugin.Instance.Localizer["bonus.damage.health.attacker", damage, abilityName]}");
                    if (victim.IsValid)
        ...
        public static bool RollDice(int chanceOfSuccess) => RollChance(chanceOfSuccess);
        ...
        {
            return RayTracer.Trace(player.EyePosition(), player.PlayerPawn.Value.GetEyeAngles(), drawResult, true);
        }
    }
    internal static class RandomProvider
    {
        ...
        internal static float NextFloat() => (float)_random.NextDouble();
    }
}
