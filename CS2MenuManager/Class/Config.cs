...
internal static class ConfigManager
{
    ...
    private const string EmbeddedConfigResourceName = "CS2MenuManager.config.toml";
    ...
    static ConfigManager()
    {
        ...
        if (string.IsNullOrWhiteSpace(assemblyDirectory))
            assemblyDirectory = AppContext.BaseDirectory ?? string.Empty;

        ConfigFilePath = Path.Combine(assemblyDirectory, "config.toml");
    }
    ...
    public static void LoadConfig()
    {
        ...
        {
            using Stream? resourceStream = CurrentAssembly.GetManifestResourceStream(EmbeddedConfigResourceName);
